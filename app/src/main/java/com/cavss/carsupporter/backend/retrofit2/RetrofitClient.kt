...
import com.cavss.carsupporter.extensions.JSONObejct.isJsonArray
import com.cavss.carsupporter.extensions.JSONObejct.isJsonObject
...
object RetrofitClient {
    ...
    // 레트로핏 클라이언트 가져오기
    fun DEF_Client(baseURL : String) : Retrofit? {
//okhttp 인스턴스 생성
        ...
        val loggingInterceptor = HttpLoggingInterceptor(object : HttpLoggingInterceptor.Logger{
            override fun log(message: String) {
                when{
                    message.isJsonObject() -> Log.e("mException", "JsonObject : ${JSONObject(message).toString(4)}")
                    message.isJsonArray() -> Log.e("mException", "JsonArray : ${JSONObject(message).toString(4)}")
                    else -> {
                        try {
//                            Log.e("mException", "Log(String) : ${JSONObject(message).toString(4)}")
                        }catch (e:Exception){
                            Log.e("mException", "except : ${e.message}")
                        }
                    }
                }
            }
...
//                    .addQueryParameter("perPage","200")
                    .addQueryParameter("serviceKey", API.KEY_DEF)
                    .build()
                ...
                if (reponse.code != 200){
                    Log.e("mException", "RetrofitClient, baseParameterInterceptor, response.code = ${reponse.code}")
                }
                return reponse
            }
        })
